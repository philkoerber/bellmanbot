...
def create_model():
    model = tf.keras.Sequential([
        tf.keras.layers.SimpleRNN(50, activation='relu', input_shape=(10, 2)),  # Input shape adjusted for bid and ask
...
def train():
    ...
    if not os.path.exists(DATA_FILE):
        return jsonify({"error": "Data file not found. Please download the data first."}), 400
